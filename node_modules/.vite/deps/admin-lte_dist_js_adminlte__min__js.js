...
var require_adminlte_min = __commonJS({
  "node_modules/admin-lte/dist/js/adminlte.min.js"(exports, module) {
    !(function(e, t) {
      ...
      class k {
        ...
        collapse() {
          ...
          if (this._parent) {
            ...
            const e3 = this._parent?.querySelectorAll(`:scope > ${L}, :scope > ${w}`);
            e3.forEach((e4) => {
      ...
      const x = ".lte.treeview", q = `expanded${x}`, T = `collapsed${x}`, $ = `load${x}`, M = "menu-open", P = ".nav-item", D = ".nav-treeview", N = { animationSpeed: 300, accordion: true };
      class F {
        _element;
        _config;
        constructor(e2, t2) {
        ...
        open() {
          ...
          if (this._config.accordion) {
            const e3 = this._element.parentElement?.querySelectorAll(`${P}.${M}`);
            e3?.forEach((e4) => {
              if (e4 !== this._element.parentElement) {
                e4.classList.remove(M);
        ...
        toggle() {
          this._element.classList.contains(M) ? this.close() : this.open();
        }
      }
      n(() => {
        document.querySelectorAll(`${P}.${M}`).forEach((e2) => {
          const t2 = e2.querySelector(D);
          if (t2) {
            o(t2, 0);
            const n2 = new Event($);
            e2.dispatchEvent(n2);
          ...
          e2.addEventListener("click", (e3) => {
            const t2 = e3.target, n2 = t2.closest(P), i2 = t2.closest(".nav-link"), o2 = n2?.querySelector(D), s2 = e3.currentTarget;
            if (o2 && ("#" !== t2?.getAttribute("href") && "#" !== i2?.getAttribute("href") || e3.preventDefault(), n2)) {
              const e4 = s2.dataset.accordion, t3 = s2.dataset.animationSpeed, i3 = { accordion: void 0 === e4 ? N.accordion : "true" === e4, animationSpeed: void 0 === t3 ? N.animationSpeed : Number(t3) };
              new F(n2, i3).toggle();
            }
          });
        });
      });
      const R = ".lte.direct-chat", C = `expanded${R}`, z = `collapsed${R}`, B = "direct-chat-contacts-open";
      class H {
        _element;
        constructor(e2) {
          this._element = e2;
        ...
        toggle() {
          if (this._element.classList.contains(B)) {
            const e2 = new Event(z);
            this._element.classList.remove(B), this._element.dispatchEvent(e2);
          } else {
            const e2 = new Event(C);
            this._element.classList.add(B), this._element.dispatchEvent(e2);
          }
        }
      }
      n(() => {
      ...
      const K = ".lte.fullscreen", I = `maximized${K}`, O = `minimized${K}`, W = '[data-lte-toggle="fullscreen"]', j = '[data-lte-icon="maximize"]', U = '[data-lte-icon="minimize"]';
      class V {
        _element;
        _config;
        constructor(e2, t2) {
        ...
        inFullScreen() {
          const e2 = new Event(I), t2 = document.querySelector(j), n2 = document.querySelector(U);
        ...
        outFullscreen() {
          const e2 = new Event(O), t2 = document.querySelector(j), n2 = document.querySelector(U);
      ...
      n(() => {
        document.querySelectorAll(W).forEach((e2) => {
          e2.addEventListener("click", (e3) => {
            ...
            t2 && new V(t2, void 0).toggleFullScreen();
          });
        });
      });
      const G = ".lte.push-menu", J = `open${G}`, Q = `collapse${G}`, X = "sidebar-mini", Y = "sidebar-collapse", Z = "sidebar-open", ee = "sidebar-expand", te = `[class*="${ee}"]`, ne = '[data-lte-toggle="sidebar"]', ie = "lte.sidebar.state", oe = { sidebarBreakpoint: 992, enablePersistence: true };
      class se {
        _element;
        _config;
        constructor(e2, t2) {
          this._element = e2, this._config = { ...oe, ...t2 };
        }
        menusClose() {
        ...
        expand() {
          ...
          document.body.classList.remove(Y), document.body.classList.add(Z), this._element.dispatchEvent(e2);
        }
        collapse() {
          const e2 = new Event(Q);
          document.body.classList.remove(Z), document.body.classList.add(Y), this._element.dispatchEvent(e2);
        }
        addSidebarBreakPoint() {
          const e2 = document.querySelector(te)?.classList ?? [], t2 = Array.from(e2).find((e3) => e3.startsWith(ee)) ?? "", n2 = document.getElementsByClassName(t2)[0], i2 = globalThis.getComputedStyle(n2, "::before").getPropertyValue("content");
          this._config = { ...this._config, sidebarBreakpoint: Number(i2.replace(/[^\d.-]/g, "")) };
          const o2 = document.body.classList.contains(Z);
          window.innerWidth <= this._config.sidebarBreakpoint ? o2 || this.collapse() : (document.body.classList.contains(X) || this.expand(), document.body.classList.contains(X) && document.body.classList.contains(Y) && this.collapse());
        }
        toggle() {
          document.body.classList.contains(Y) ? this.expand() : this.collapse(), this.saveSidebarState();
        }
        saveSidebarState() {
          if (this._config.enablePersistence && void 0 !== globalThis.window && void 0 !== globalThis.localStorage) try {
            const e2 = document.body.classList.contains(Y) ? Y : Z;
            localStorage.setItem(ie, e2);
          } catch {
          }
        }
        loadSidebarState() {
          if (this._config.enablePersistence && void 0 !== globalThis.window && void 0 !== globalThis.localStorage && !(globalThis.innerWidth <= this._config.sidebarBreakpoint)) try {
            const e2 = localStorage.getItem(ie);
            e2 === Y ? this.collapse() : e2 === Z && this.expand();
          } catch {
          }
        }
        init() {
          this.addSidebarBreakPoint(), this.loadSidebarState();
        }
      }
      n(() => {
        const e2 = document?.querySelector(".app-sidebar");
        if (e2) {
          const t3 = new se(e2, oe);
          t3.init(), window.addEventListener("resize", () => {
            t3.init();
          });
        }
        const t2 = document.createElement("div");
        t2.className = "sidebar-overlay", document.querySelector(".app-wrapper")?.append(t2);
        let n2 = false;
        t2.addEventListener("touchstart", () => {
          n2 = false;
        }, { passive: true }), t2.addEventListener("touchmove", () => {
          n2 = true;
        }, { passive: true }), t2.addEventListener("touchend", (e3) => {
          if (!n2) {
            e3.preventDefault();
            const t3 = e3.currentTarget;
            new se(t3, oe).collapse();
          }
          n2 = false;
        }, { passive: false }), t2.addEventListener("click", (e3) => {
          e3.preventDefault();
          const t3 = e3.currentTarget;
          new se(t3, oe).collapse();
        }), document.querySelectorAll(ne).forEach((e3) => {
          e3.addEventListener("click", (e4) => {
            e4.preventDefault();
            let t3 = e4.currentTarget;
            "sidebar" !== t3?.dataset.lteToggle && (t3 = t3?.closest(ne)), t3 && (e4?.preventDefault(), new se(t3, oe).toggle());
          });
        });
      });
      class ae {
        ...
        initFormAccessibility() {
          document.querySelectorAll("input, select, textarea").forEach((e2) => {
            ...
            t2.classList.contains("disable-adminlte-validations") || t2.addEventListener("invalid", () => {
      ...
      const re = (e2) => new ae(e2);
      n(() => {
        new a(document.body).holdTransition(), re({ announcements: true, skipLinks: true, focusManagement: true, keyboardNavigation: true, reducedMotion: true }).addLandmarks(), setTimeout(() => {
          document.body.classList.add("app-loaded");
        }, 400);
      }), e.CardWidget = k, e.DirectChat = H, e.FullScreen = V, e.Layout = a, e.PushMenu = se, e.Treeview = F, e.initAccessibility = re;
    });
  }
});
